...
6. **หลังแก้เสร็จ รัน `npm run verify` ให้ผ่านก่อน push ทุกครั้ง** (ไม่ใช่แค่ `build`)
   `verify` = lint + build + `check:text` + `check:images` + `check:citations` ซึ่งอ่าน
...
| บทสรุปอีก 8 ชิ้น + ประกาศแผนวิจัย 2 ชิ้น | #31 | รวมเป็น 28 จาก 75 · เพิ่ม `kind: "protocol"` ให้หน้าแผนวิจัยเรียกหัวข้อว่า "จะทำอะไร" และขึ้นป้ายบอกว่ายังไม่มีผล — **ตามที่ผู้ใช้เสนอเมื่อ 2 ก.ย. ว่าแผนวิจัยก็ควรมีหน้าของตัวเอง เพียงแต่ต้องเรียกให้ตรงกับสิ่งที่มันเป็น** · ภาพประกอบ paper-craft อีก 7 ใบ |
