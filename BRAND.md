...
**Version 1.3 · September 2026** (v1.2 typography → Kanit 400/500 + two-tone display · v1.3 เพิ่มข้อยกเว้นฟอนต์สำหรับอักษรจีน)
...
**ข้อยกเว้นเดียว (v1.3 · 24 ก.ย. 2569 · ผู้ใช้อนุมัติ): ฟอนต์เสริมสำหรับอักษรที่ Kanit ไม่มี — Noto Sans SC 400/500**
- ใช้**เฉพาะ route group `(zh)`** ผ่านกฎ CSS ที่ผูกกับ `html[lang="zh-Hans"]` เท่านั้น · หน้าไทยและอังกฤษไม่โหลดไฟล์ฟอนต์ชุดนี้เลย
- อยู่**หลัง Kanit** ในลำดับ font-family เสมอ — ตัวเลข ละติน และเครื่องหมายวรรคตอนละตินยังเป็น Kanit ทุกหน้า มีแต่อักษรจีนที่ตกมาถึงฟอนต์นี้
- **ห้ามใช้กับข้อความไทยหรืออังกฤษ** และ**ห้ามเพิ่มน้ำหนักอื่น**นอกจาก 400/500 — กฎ type scale ข้างล่างใช้กับหน้าจีนเหมือนเดิมทุกข้อ
- self-host ผ่าน `next/font/google` เหมือน Kanit (`src/lib/fontsZh.ts` — แยกไฟล์จาก Kanit เพื่อให้ CSS ของฟอนต์จีน 185 KB ถูกลิงก์เฉพาะหน้าจีน) จึงไม่เพิ่มโดเมนใน CSP · เหตุผลที่ไม่ปล่อยให้ระบบเลือกฟอนต์จีนเอง: Windows ไม่มีน้ำหนัก 500 ให้ Microsoft YaHei หัวเรื่องจะหนาเท่าเนื้อหา
