...
export default async function EnglishCaseStudyPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen">
          ...
          {usedServices.length > 0 && (
            <div className="flex flex-wrap items-center gap-x-2 gap-y-2 text-[15px] leading-[1.6]">
              ...
              {usedServices.map(({ service, stage }) => (
                <Link
                  key={service.key}
                  href={`/en/expertise#stage-${stage.key}`}
      ...
      {/* Partner voice — the whole block is dropped when there is nothing to quote */}
      {copy.testimonial && (
        <section className="mx-auto max-w-3xl px-6 py-16">
          <ProjectTestimonial testimonial={copy.testimonial} accent={goal.deep} locale="en" />
        </section>
      )}
