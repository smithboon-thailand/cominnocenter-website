...
/**
 * ตัวเลขผลลัพธ์ที่แสดงเป็น Stat บนหน้ารายละเอียดโครงการ (Phase 6.2)
 *
 * กฎเดียวกับผลงานวิชาการ (CLAUDE.md ข้อ 8): ใส่ได้เฉพาะตัวเลขที่ยืนยันได้จริง
 * ห้ามเอา "เป้าหมาย" มาแสดงเป็น "ผลลัพธ์" เช่น Chula Zero Waste ตั้งเป้าลดขยะ 30%
 * ซึ่งเป็นเป้าของแผนมหาวิทยาลัย ไม่ใช่ผลที่แคมเปญของศูนย์ฯ วัดได้ จึงไม่ใส่
 * โครงการที่ยังไม่มีตัวเลขให้เว้นว่าง — หน้าเว็บซ่อนส่วนนี้ทั้งก้อน ไม่ขึ้น placeholder
 */
export type ProjectResult = {
  /** ตัวเลขตามที่จะแสดง ใส่ comma/+ มาได้ เช่น "10,000+" */
  value: string;
  /** หน่วย เช่น "คน", "เดือน" — ผูกกับตัวเลข ไม่หลุดบรรทัด */
  unit?: string;
  label: string;
};

/** เสียงจากพันธมิตร — optional เสมอ ต้องได้รับอนุญาตจากผู้พูดก่อนขึ้นเว็บ */
...
export type Project = {
  ...
  approach: string;
  impact: string;
  /** ตัวเลขผลลัพธ์ — มีเฉพาะโครงการที่ยืนยันตัวเลขได้ (ดู ProjectResult) */
...
export const projects: Project[] = [
  ...
  },
  {
    ...
    results: [
      { value: "100", unit: "คน", label: "นวัตกรที่รวบรวมและเล่าเรื่อง" },
      { value: "3", unit: "ปี", label: "ระยะเวลาดำเนินโครงการ" },
    ],
  ...
  },
  {
    ...
    results: [
      { value: "3", unit: "จังหวัด", label: "พื้นที่ชายแดนใต้ที่จัดอบรม" },
    ],
  ...
  },
  {
    ...
    results: [
      { value: "10,000+", unit: "คน", label: "บุคลากรสาธารณสุขที่ผ่านการอบรม" },
    ],
  ...
  },
  {
    ...
    results: [
      { value: "12", unit: "เดือน", label: "ระยะเวลาหลักสูตรออนไลน์" },
    ],
  ...
  },
  {
    ...
    results: [
      { value: "10", unit: "คน/รุ่น", label: "ขนาดกลุ่มผู้เรียนต่อรุ่น" },
    ],
