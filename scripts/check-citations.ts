...
for (const p of citable) {
  ...
  for (const [style, text] of [
    ...
    /**
     * จุดไข่ปลาของผู้เขียนตั้งแต่ 21 คนขึ้นไปเป็น**ส่วนหนึ่งของรูปแบบ** ไม่ใช่ความผิด
     *
     * APA 7 §9.8 บอกให้ลงชื่อ 19 คนแรก ตามด้วยจุดไข่ปลา แล้วปิดท้ายด้วยคนสุดท้าย
     * ผลลัพธ์จึงมี ", ... " อยู่กลางรายชื่อโดยถูกต้อง — ปิดบังไว้ก่อนตรวจ
     * ไม่ใช่ผ่อนกฎ เพราะกฎยังจับ ".." ที่เกิดจากการต่อสตริงผิดได้เหมือนเดิมทุกกรณี
     *
     * เจอ 8 ก.ย. 2569 ตอนนำเข้าผลงานของ ดร.พยู ฮนิน ไหล่ ซึ่งเป็นงานสองชิ้นแรก
     * ในคลังที่มีผู้เขียนเกิน 20 คน เส้นทางนี้จึงไม่เคยถูกตรวจมาก่อน
     */
    const probe =
      style === "APA" && c.authors.length > 20 ? text.replace(/,\s\.\.\.\s/g, ", ") : text;

    // เครื่องหมายวรรคตอนซ้อน — อาการของการต่อสตริงผิด ไม่ใช่ของที่ทะเบียนส่งมา
    if (/\.\.|,,|\?\.|!\.|\s,|\s\.(?!\s*$)/.test(probe))
      flag(p, style, "เครื่องหมายวรรคตอนซ้อน", text);
