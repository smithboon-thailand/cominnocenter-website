...
// ─── 3ข. ลิงก์ที่ไม่มีข้อความข้างใน ───────────────────────────────────────────
//
// `<a href="…"></a>` ที่ไม่มีอะไรอยู่ข้างในกินพื้นที่ศูนย์พิกเซล ผู้อ่านจึงไม่เห็น
// และกดไม่ได้ — เท่ากับหน้านั้น**ไม่มีลิงก์นั้นอยู่จริง** แม้ href จะถูกต้องทุกตัวอักษร
//
// เคยพลาดจริง (3 ก.ย. 2569): ลิงก์อีเมลบน /collaborate และ /privacy-policy
// ทั้งสองภาษา **หกจุด** เป็น `<a>` เปล่าอยู่บนเว็บ production ที่อยู่อีเมลของศูนย์ฯ
// จึงไม่ปรากฏบนสองหน้านั้นเลย · หน้านโยบายหนักที่สุดเพราะประโยค "ใช้สิทธิเหล่านี้ได้
// โดยเขียนมาที่ ___" กลายเป็นประกาศทางกฎหมายที่ชี้ไปที่ว่างเปล่า ซึ่งเป็นปัญหา
// เดียวกับที่เว้นว่างในข้อ 3 ทุกประการ เพียงแต่มองจาก HTML ไม่ใช่จากซอร์ส
//
// **อาการที่หลอกคนตรวจ**: TypeScript ไม่ฟ้อง เพราะ JSX ยอมให้ element ว่างได้
// สิ่งเดียวที่ส่งเสียงคือคำเตือน eslint ว่า `EMAIL` ถูก import มาแล้วไม่ได้ใช้ —
// ซึ่งอ่านผ่านๆ เหมือน import เกิน จึงถูกมองข้ามมาหลายรอบ ทั้งที่**เป็นอาการ
// ของข้อความที่หายไป ไม่ใช่ของ import ที่เกินมา**
//
// ยกเว้นลิงก์ที่เจตนาไม่มีข้อความแต่มีชื่อเรียกอย่างอื่นให้ screen reader อยู่แล้ว
// (aria-label / title) และลิงก์ที่ห่อไอคอนหรือรูปภาพไว้ข้างใน
const EMPTY_LINK = /<a\b([^>]*)>(\s*)<\/a>/g;

for (const file of shipped) {
  if (!file.endsWith(".html")) continue;
  const body = readText(file);
  if (body === null) continue;
  for (const m of body.matchAll(EMPTY_LINK)) {
    const attrs = m[1];
    if (/\baria-label=|\btitle=|\baria-labelledby=/.test(attrs)) continue;
    const href = attrs.match(/href="([^"]*)"/)?.[1] ?? "(ไม่มี href)";
    errors.push(
